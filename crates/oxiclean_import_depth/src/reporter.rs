...
pub fn print_no_depth_issues_message<W: Write>(writer: &mut W, threshold: usize) -> io::Result<()> {
    debug!("No depth issues detected");
    writeln!(
        writer,
        "{} No excessive import depth detected. Threshold: {}",
        "✓".green().bold(),
        threshold
    )?;
    writer.flush()?;
...
pub fn print_warnings_tree<W: Write>(
    ...
    writeln!(
        writer,
        "{} Excessive import depth detected (threshold: {} levels)\n",
        "⚠".yellow().bold(),
    ...
    for file in files {
        ...
        let display_path = if let Ok(root) = cfg.root() {
